...
/// Flags that control how `inotify_add_watch` resolves the path or merges with an existing watch,
/// rather than which events it reports. They belong to a single add call and must never be stored
/// in [`Watch::watch_mask`], which is merged into later adds for the same path.
...
struct Watch {
    ...
    is_dir: bool,
    /// Whether the watch observes the destination of the path rather than the path itself. #255
    dereference: bool,
...
impl EventLoop {
    ...
    fn add_watch(
        ...
        // Watching the path itself and walking what it points at are exclusive, so a recursive
        // watch resolves the path as usual. #255
    ...
    {
        for path in paths {
            // Only the root of the walk carries the caller's choice; entries below it were reached
            // by following links, so they observe what they resolved to. #255
        ...
        if let Some(ref mut inotify) = self.inotify {
            ...
            match inotify.watches().add(&path.absolute, add_mask) {
                ...
                Ok(w) => {
                    ...
                    // Re-resolving a path can land on a different inode, and so a different
                    // descriptor, than the watch it replaces: dropping `DONT_FOLLOW` moves the
                    // watch from a link to its destination and adding it moves it back. Release
                    // the old descriptor so it does not keep reporting under this path. #255
                    if let Some(previous) = previous_descriptor.filter(|previous| *previous != w) {
                        // A walk that followed this link also watched its destination, and both
                        // spellings share the old descriptor, so only release it if no other
                        // watched path still resolves to it. #255
...
mod tests {
    ...
    /// Create a watcher that only reports data changes, so exact event assertions are not disturbed
    /// by access events.
    ...
    /// A recursive watch resolves the path it walks, so asking for the link itself cannot silently
    /// turn a recursive watch into a watch that reports nothing.
    #[test]
    ...
    #[test]
    fn a_recursive_walk_does_not_overrule_an_explicit_dereference_choice() {
        let tmpdir = tempfile::tempdir().unwrap();
        let root = tmpdir.path().to_path_buf();
    ...
    /// Moving a watch onto the link itself must not tear down the coverage an ancestor recursive
    /// watch has of the link's destination. #255
    #[test]
    fn watching_a_link_itself_keeps_the_ancestors_recursive_coverage() {
        ...
        assert!(
            !reported.is_empty(),
            "the ancestor recursive watch stopped reporting the link's destination"
        );
    }

    /// Asking for the link itself does take the watch off the destination when the link is the
    /// only way the tree reaches it. The caller asked not to follow that path, and there is no
    /// other spelling under the recursive root to keep reporting it. #255
    #[test]
